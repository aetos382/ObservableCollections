...
This rule holds unconditionally, but whether it can be enforced depends on when the handler runs. A view re-raises the change on its own `CollectionChanged` through an `ICollectionEventDispatcher`, and the dispatcher decides that timing. The default one invokes the handler inline, and `SynchronizationContextCollectionEventDispatcher` sends synchronously when it is already on the target context - which is what happens when you change the collection from the UI thread. In both cases the handler still runs inside the source collection's notification, so a change made there is refused just as above. Only a dispatcher that posts the notification elsewhere, to another thread or to a later turn of the message loop, runs the handler after the source has finished notifying, and only then is the change applied rather than refused. Because that depends on which thread the change originated from, do not rely on either outcome: a change from a notification handler is a rule violation whether or not it is caught.
